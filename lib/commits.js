...
const loadCommits = async (repoPath, rev) => {
  ...
  try {
    ({ stdout } = await promisify(execFile)(
      'git',
      [`--git-dir=${repoPath}`, 'rev-list', `--pretty=format:${format}`, '--header', rev || 'HEAD'],
      {
...
export default async (tags, options = {}) => {
