...
const increment = async (type, preSuffix, dryRun = false) => {
...
export const createAnnotatedTag = async (tag, changelog, summary = '') => {
  const tagName = typeof tag === 'string' ? tag : tag.tag;
  const messageArgs = ['-m', changelog];

  if (summary) {
    messageArgs.push('-m', `Update-Summary: ${summary}`);
  }

  createSpinner('Tagging commit');

  try {
    await runGitArgs(['tag', '-a', tagName, ...messageArgs]);
  } catch (err) {
    fail(err.message);
  }
...
export default async (type, preSuffix, tagOnly = false, dryRun = false) => {
  createSpinner('Bumping version tag');
  let release;

  try {
    release = await increment(type, preSuffix, dryRun);
  } catch (err) {
    fail(err.message);
  }
  ...
  if (dryRun) {
    ...
    global.spinner.succeed(`Would bump version tag to ${chalk.bold(tag)}`);
    global.spinner = null;
